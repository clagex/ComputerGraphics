...
void Scene::loadCore() {
	...
	m_boids.at(0).clear_prey(); // reset the prey in case the address is assigned again 
}
...
void Scene::loadCompletion() {
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 120; j++) {
			m_boids.push_back(Boid(linearRand(-m_bound_hsize, m_bound_hsize), sphericalRand(1.0), i));
		}
	}
	m_boids.push_back(Boid(linearRand(-m_bound_hsize, m_bound_hsize), sphericalRand(1.0), 2, true));
}
...
void Scene::loadChallenge() {
	...
	m_boids.at(0).clear_prey(); // reset the prey in case the address is assigned again 
	for (int i = 0; i < 3; i++) {
		float size = linearRand(3.0, 8.0);
		int pos = 300 + i;
		m_boids.push_back(Boid(linearRand(-m_bound_hsize+vec3(size), m_bound_hsize-vec3(size)), size, true));
		//m_boids.push_back(Boid(vec3(0), size, true));
		m_objs.push_back(m_boids.at(pos));
	}
}
...
void Scene::draw(const mat4 &proj, const mat4 &view) {
	...
	for (const Boid &b : m_boids) {
		...
		mat4 transMatrix = translate(mat4(1), b.position());
		model = scale(transMatrix, vec3(b.scale())) * model;
...
void Scene::renderGUI() {
	...
	ImGui::Text("Speeds");
	ImGui::SliderFloat("min. speed", &m_min_speed, 0.0, 6.0, "%.1f");
	ImGui::SliderFloat("max. speed", &m_max_speed, 0.0, 10.0, "%.1f");
	ImGui::SliderFloat("max. accel", &m_max_accel, 0.0, 5.0, "%.1f");
	
	ImGui::Separator();
	static const char * modes[]{ "Wrap", "Bounce"};
	ImGui::Text("Boundary");
	...
	ImGui::SliderFloat("Neighbourhood Radius", &m_rad, 0.0, 50.0, "%.1f");
	ImGui::SliderFloat("Cohesion", &m_cohes, 0.0, 5.0, "%.1f");
	ImGui::SliderFloat("Alignment", &m_align, 0.0, 5.0, "%.1f");
	ImGui::SliderFloat("Avoidance", &m_avoid, 0.0, 5.0, "%.1f");
	ImGui::SliderFloat("Anti-Predator", &m_flee, 0.0, 100.0, "%.1f"); // boid anti-predator weight

	ImGui::Separator();
	ImGui::Text("Predator Parameters");
	ImGui::SliderFloat("min. speed.", &m_pred_min_speed, 0.0, 6.0, "%.1f");
	ImGui::SliderFloat("max. speed.", &m_pred_max_speed, 0.0, 10.0, "%.1f");
	ImGui::SliderFloat("max. accel.", &m_pred_max_accel, 6.0, 15.0, "%.1f");
	ImGui::SliderFloat("Pursuit", &m_pursuit, 0.0, 1.0, "%.1f");
	ImGui::SliderFloat("Neighbourhood radius", &m_pred_rad, 20.0, 70.0, "%.1f");
	ImGui::SliderFloat("Chasing", &m_chasing, 0.0, 10.0, "%.1f"); // chase weight
	ImGui::SliderFloat("Avoidance.", &m_pred_avoid, 0.0, 5.0, "%.1f"); 

	ImGui::Separator();
	ImGui::Text("Object Parameters");
	ImGui::SliderFloat("Boid radius", &m_obj_rad, 0.0, 10.0, "%.1f");
	ImGui::SliderFloat("Avoidance weight", &m_obj_avoid, 0.0, 100.0, "%.1f");

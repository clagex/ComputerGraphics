...
float Scene::m_diff = 0.55;
float Scene::m_spec = 0.39;
...
Scene Scene::shapeScene() {
	...
	shared_ptr<Texture> tex = make_shared<Texture>(CGRA_WORKDIR+ string("//res//textures//checkerboard.jpg"));
	shared_ptr<Material> tex_mat = make_shared<Material>(vec3(1), 1.05f, 0.1, 0, tex);

	objects.push_back(make_shared<SceneObject>(make_shared<AABB>(vec3(-3, 0, -5), vec3(0.5)), tex_mat));
	objects.push_back(make_shared<SceneObject>(make_shared<Sphere>(vec3(-1, 0, -5), 0.5), tex_mat));

	// YOUR CODE GOES HERE
	objects.push_back(make_shared<SceneObject>(make_shared<Plane>(vec3(0, -1, -5), vec3(0, 1, 0)), tex_mat));
	objects.push_back(make_shared<SceneObject>(make_shared<Disk>(vec3(1, 0, -4), vec3(0.7, 0, 1.0f), 0.7f), tex_mat));
	objects.push_back(make_shared<SceneObject>(make_shared<Triangle>(vec3(2.5, 1, -5), vec3(2.5, -1, -5), vec3(3.5, -1, -5)), tex_mat));

...
public:
	...
	Material(const glm::vec3& diffuse_chroma, float shininess, float specular_ratio, float metalicity_ratio, std::shared_ptr<Texture> tex);
	...
	virtual std::shared_ptr<Texture> texture() const { return m_texture; }

	// return whether the material has texture
	virtual bool hasTexture() const { return m_has_tex; }
};
...
void Application::render() {
	...
	m_cameraPos.y = sin(m_pitch) * m_distance;
	m_cameraPos.x = cos(m_yaw) * cos(m_pitch) * m_distance;
	m_cameraPos.z = sin(m_yaw) * cos(m_pitch) * m_distance;
	mat4 view = lookAt(m_cameraPos, m_cameraTarget, up); // TODO replace view matrix with the camera transform
...
void Application::renderGUI() {
	...
	ImGui::SliderAngle("Camera yaw", &m_yaw);
	ImGui::SliderAngle("Camera pitch", &m_pitch);
